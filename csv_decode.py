...
for year in range(1977,2024):
    ...
    # csvの読み込み
    df = pd.read_csv("./WTHOR_csv/"+str(year)+".csv")
    ...
    # np.save("./model/x/"+str(year),x_train)
    # np.save("./model/y/"+str(year),y_train_reshape)
    if year == 1977:
        x_con = x_train
        y_con = y_train_reshape
    else :
        x_con = np.concatenate([x_con,x_train])
        y_con = np.concatenate([y_con,y_train_reshape])
    print(year,"done")

np.save("./model/x/all",x_con)
np.save("./model/y/all",y_con)